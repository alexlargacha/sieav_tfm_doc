...
En primer lugar, Jailhouse se declara más preocupado del aislamiento y segmentación que de la virtualización. Se denomina \textit{hipervisor de particionamiento}. Jailhouse es muy ligero y no proporciona muchas de las funcionalidades que tradicionalmente proporcionan los hipervisores como pueden ser el sobreutilizamiento de recursos. Por ejemplo no ofrece vCPUs a los sistemas guest y no emula dispositivos que no tiene, por lo que no necesita de un scheduler que vaya repartiendo los recursos.\\
Jailhouse proporciona
