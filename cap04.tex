...
Primero seleccionar Xen dentro dentro del grupo de paquetes a incluir en el sistema de ficheros.
\begin{lstlisting}[style=CStyle]
$ petalinux-config -c rootfs
\end{lstlisting}

Aparece un menú en el que hay que navegar hasta la ventana que se muestra en la figura \ref{fig:xen_menuconfig_1}:
\begin{lstlisting}[style=CStyle]
...
\begin{figure*}[!h]
  \centering
  \includegraphics[width=0.90\textwidth]{recursos/petalinux_xen_1.png}
  \caption{Configuración de paquetes en el sistema de ficheros}
  \label{fig:xen_menuconfig_1}
\end{figure*}


Alternativamente a la generación mediante PetaLinux, exite siempre la posibilidad de descargar el código fuente y compilarlo. Para ello, se deben ejecutar los siguientes comandos \cite{xen_source_xilinx}
